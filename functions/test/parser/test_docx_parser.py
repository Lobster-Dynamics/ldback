...
def test_returns_correct_type(assets_dir: str, parser: DOCXParser):
    file_bytes = _readfile(f"{assets_dir}/doc1.docx")
    res = parser.parse(file_bytes)
    assert isinstance(res, ParsingResult)


def test_all_images_extracted_from_doc1(assets_dir: str, parser: DOCXParser):

    file_bytes = _readfile(f"{assets_dir}/doc1.docx")
    res = parser.parse(file_bytes)
    assert len(res.image_sections) == 3


def test_correct_indexes_of_sections(assets_dir: str, parser: DOCXParser):
    file_bytes = _readfile(f"{assets_dir}/doc1.docx")
    res = parser.parse(file_bytes)
