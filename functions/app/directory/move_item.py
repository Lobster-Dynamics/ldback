...
def move_file_handle():
    ...
    try:
        ...
        return jsonify(msg="Directory id, New directory id and Item id must be set"), 400
    ...
    if (not curr_dir.owner_id == token["uid"] 
        or not new_dir.owner_id == token["uid"]
        or (item.item_type == "DOCUMENT" and str(doc_repo.get_reduced(str(item.item_id)).owner_id) != token["uid"])
        or (item.item_type == "DIRECTORY" and str(repo.get_reduced(str(item.item_id)).owner_id) != token["uid"])):
